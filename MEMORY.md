...
### 2026-07-19 — Scraper mis-extracted client-rendered pages: identical 41-service lists + slug collisions (Backend)
**Context:** QA of the live app found two real defects. (1) Many different architectures stored the **same 41 AWS services** (including irrelevant ones like SageMaker/Neptune for an EFS file manager). (2) The scrape job **always** lost exactly one page — "Connected Mobility Solution on AWS" failed on a Mongo `duplicate key error` for `slug: "aws-solutions-library"` (19/20 parsed every run). Root cause was shared: AWS article pages are **client-rendered**, so the fetched HTML exposes only site chrome — the `_extract_services` raw-HTML scan matched the global service nav embedded in `<script>` (41 on every page), and `_extract_title` fell back to the generic section title "AWS Solutions Library" for several pages, producing a colliding slug under the unique index.
**Decision / Fix:** The directory API (already used for discovery) is authoritative and returns each item's real `headline` **and** `description`. Thread that through the pipeline as a new `SourceHints(title, description)` (added to the `ArchitectureParser.parse` signature, optional, plumbed through rules + LLM parsers); the parser prefers hints over HTML extraction. For services, strip site chrome (`<script>/<style>/<nav>/<header>/<footer>`) via new `_strip_chrome` **before** scanning, so only real content counts. Verified live: fresh scrape → 20/20 parsed, 0 failures, 28 distinct slugs (lost page recovered), and page-specific service lists (0–19, not a uniform 41).
**Why / Lesson:** Do **not** trust `<h1>`/`<title>`/raw-HTML scans on client-rendered pages — they yield generic chrome. When a discovery source already carries authoritative metadata, plumb it through rather than re-deriving it from the page. Scanning *raw* HTML for services matches the global nav; always scope detection to stripped main content. A consequence to accept: some client-rendered pages legitimately expose few/zero services in static HTML — honest and page-specific beats rich-but-fake (characteristics still derive from the authoritative title+description). A headless renderer would be the next step if richer per-page services are ever required.
